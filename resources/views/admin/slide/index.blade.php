...
<div class="br-pageheader">
    <nav class="breadcrumb pd-0 mg-0 tx-12">
      <a class="breadcrumb-item" href="{{route('admin.slider')}}">Quản lý silder</a>
    </nav>
</div>
<div class="br-pagetitle" style="justify-content: space-between;">
	<h4>Danh sách slides</h4>
	<div class=""><a href="{{ route('admin.slider.add') }}" class="btn btn-success">Thêm mới</a></div>
</div>
		                ...
		                <tr class="thead-colored thead-light">
				      	...
				      	@foreach ($listSlide as $item)
							<tr id="record_5">
								<td>{{ $item->id }}</td>
					          	<td>{{ $item->title }}</td>
					          	<td><img width="100" height="100" src="{{ asset('storage/slider/' . $item->image) }}" ></td>
					          	<td>{{ $item->link }}</td>
					          	<td>{{ $item->position }}</td>
					          	<td>{{ $item->active }}</td>
					          	<td class="text-center">
					          		<a href="{{route('admin.slider.edit', ['id' => $item->id])}}" class=""><i class="fas fa-edit"></i> Sửa</a>
					          	</td>
					          	<td class="text-center">
									<a style="font-size: 15px; color: red" class="buttonDelteRecord" href="{{route('admin.slider.remove', ['id' => $item->id])}}"><i class="fa fa-times-circle"></i> Xóa</a>
					          	</td>
					        </tr>
				      	@endforeach
		      		</tbody>
