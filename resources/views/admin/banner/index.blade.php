...
<div class="br-pageheader">
    <nav class="breadcrumb pd-0 mg-0 tx-12">
      <a class="breadcrumb-item" href="{{route('admin.banner')}}">Quản lý banner</a>
    </nav>
</div>
<div class="br-pagetitle" style="justify-content: space-between;">
	<h4>Danh sách banner</h4>
	<div class=""><a href="{{ route('admin.banner.add') }}" class="btn btn-success">Thêm mới</a></div>
</div>
		  	...
		  	<div class="bd rounded table-responsive">
		  	  	<table class="table table-bordered table-striped mg-b-0">
		                ...
		                <tr class="thead-colored thead-light">
		                	...
		                	<td class="wd-5p">Vị trí</td>
							<td class="wd-5p">Kích hoạt</td>
							<td class="wd-5p">Ngày đăng</td>
				      	...
				      	@foreach ($listBanner as $item)
							<tr id="record_5">
								<td>{{ $item->id }}</td>
					          	<td>{{ $item->name }}</td>
					          	<td><img width="100" height="100" src="{{ asset('storage/banner/' . $item->image) }}" ></td>
					          	<td>{{ $item->link }}</td>
					          	<td>{{ $item->position }}</td>
					          	<td>{{ $item->active }}</td>
					          	<td>{{ $item->date }}</td>
					          	<td class="text-center">
					          		<a href="{{route('admin.banner.edit', ['id' => $item->id])}}" class=""><i class="fas fa-edit"></i> Sửa</a>
					          	</td>
					          	<td class="text-center">
									<a style="font-size: 15px; color: red" class="buttonDelteRecord" href="{{route('admin.banner.remove', ['id' => $item->id])}}"><i class="fa fa-times-circle"></i> Xóa</a>
					          	</td>
					        </tr>
				      	@endforeach
		      		</tbody>
