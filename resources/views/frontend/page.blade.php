...
<head>
  ...
  <meta property="og:title" content="{{ $page_detail->meta_title }}" />
  <meta property="og:description" content="{{ $page_detail->meta_description }}" />
  ...
  <div class="bg-color-b">
    <div class="row">
      <div id="content" class="col-sm-9">
          <p class="heading">{{ $page_detail->name }}</p>
          ...
          <div class="col-sm-3">
            <aside id="column-left" class="col-xs-12">
              <div class="list-group cateleft">
                <label class="list-group-item">Danh sách bài viết</label>
                @foreach($page as $item)
                  <a href="{{route('page', $item->id)}}" class="list-group-item"><i style="font-size: 11px;" class="fa fa-chevron-right" aria-hidden="true"></i>&nbsp{{$item->name}}</a>
                @endforeach
              </div>
            </aside>
          </div>
