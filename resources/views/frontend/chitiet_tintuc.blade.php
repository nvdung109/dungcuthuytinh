...
<div class="back">
  <ul class="breadcrumb">
    ...
    <li><a href="{{ url('tin-tuc', $news->url)}}">{{ $news->title }}</a></li>
  </ul>
</div>
@if(session('Comment'))
...
<div class="container">
  <div class="bg-color-b">
    <div class="row">
      <div id="content" class="col-sm-12 inspire-single-blog">
        <div class="row">
          <div class="col-md-9 col-xs-12 news-detail">
            <h1 style="color: #00aeef" class="title">{{ $news->title }}</h1>
            <div class="blog_links">
              <div>
                <span class="blog-date">{{ $news->date }}</span>
              </div>
            </div>
            <div class="blog-desc-en">{!! $news->desc_en !!} </div>
            <div class="blog-desccription">{!! $news->description !!} </div>
            <div class="block-title">
              <p class="heading">Bình luận - Đánh giá</p>
            </div>
            <div class="panel panel-default" id="add-comment-blog" style="margin-bottom: 10px;">
              <form action="comment-news/{{$news->id}}" method="post" enctype="multipart/form-data" class="form-horizontal">
                @csrf
                ...
                <div class="form-group required" style="margin-top: 10px;">
                  <div>
                    ...
                    <input type="text" name="telephone" value="{{old('telephone')}}" placeholder="Nhập số điện thoại" class="form-control" />
                ...
                <div class="buttons text-left">
                  <input class="btn btn-primary" type="submit" value="Gửi bình luận" data-toggle="tooltip" title="Submit" />
                </div>
              </form>
              @foreach($cmt_news as $item)
                <div class="comment-rel col-lg-12 col-sm-12 col-12">
            ...
            <div class="rel-news">
              <p class="heading">Tin tức liên quan</p>
              <div class="row">
                @foreach($news_tt as $item)
                <div class="col-lg-4 col-sm-12">
                  <div class="box-news">
                      <a href="{{ url('tin-tuc', $item->url) }}"><img src="{{ asset('storage/news/' .$item->image) }}" alt="{{$item->name}}" class="img-responsive"></a>
                      <a href="{{ url('tin-tuc', $item->url) }}">
                        <h3 class="new-name">{{ $item->title }}</h3>
                      </a>
                      <div class="blog_links">
                        <div>
                      ...
                      <div class="blog-desc">{!! $item->desc_end !!}</div>
          ...
          <div class="sidebar-new">
            <div class="row">
              <div class="col-sm-3">
                <div style="margin-bottom: 20px" class="list-group cateleft">
                  <label class="list-group-item">Danh mục tin tức</label>
                  @foreach($cate_news as $item)
                    <a href="{{ route('danhmuctintuc', $item->id) }}" class="list-group-item"><i style="font-size: 11px;" class="fa fa-chevron-right" aria-hidden="true"></i> {{ $item->name }}</a>
                  @endforeach
                </div>
                <div style="margin-bottom: 20px" class="banner">
                  <img width="100%" height="auto" src="giaodien/image/catalog/banner-news1.jpg" alt="">
                </div>
                <aside id="column-left" class="col-xs-12 ">
                  <div class="new-hot">
                    <p class="list-group-item">Tin mới nhất</p>
                    @foreach($new_hot as $item)
                    <div class="box-hot">
                      <div class="image">
                        <a href="{{ url('tin-tuc', $item->url) }}">
                          <img src="{{ asset('storage/news/' .$item->image) }}" alt="" title="" class="img-responsive center-block" />
                        </a>
                        <span class="blog-date"><i class="fa fa-calendar" aria-hidden="true"></i> &nbsp{{ $item->date }}</span>
                        <p class="new-name">
                          <a href="{{ url('tin-tuc', $item->url) }}">{{$item->title}}</a>
                        </p>
                      </div>
                    </div>
                    @endforeach
